...
let helpers = {
    ...
    triggerMatchRules: [
        ...
        {text: "HasEventsMatchRegex(REGEX)", displayText: "HasEventsMatchRegex(regex string) bool | 判断是否有匹配正则表达式的事件"},
        {text: "HasEventsMatchRegexs([]REGEX)", displayText: "HasEventsMatchRegexs(regex []string) bool | 判断是否有匹配正则表达式（多个或关系）的事件"},
        ...
        {text: "UsersHasPropertyRegex(KEY, VALUE_REGEX, RETURN)", displayText: "UsersHasPropertyRegex(key, valueRegex string, returnField string) []string | 根据字段、值查询用户，返回值指定字段列表，字段值为正则表达式匹配"},
        {text: "UsersIDWithProperty(KEY, VALUE, RETURN)", displayText: "UsersIDWithProperty(key, value, returnField string) []repository.UserIDWithMeta | 根据字段、值查询用户，返回值指定字段和id"},
        {text: "UsersIDWithPropertyRegex(KEY, VALUE_REGEX, RETURN)", displayText: "UsersIDWithPropertyRegex(key, valueRegex, returnField string) []repository.UserIDWithMeta | 根据字段、值查询用户，返回值指定字段和id，字段值为正则表达式匹配"},
    ...
    templates: [
        ...
        {text: 'line_filter_includes STR FILTERS', displayText: 'line_filter_includes(val string, filters ...string) string  | 字符串按照行过滤，保留匹配的行，支持多个 filter'},
        {text: 'line_filter_excludes STR FILTERS', displayText: 'line_filter_excludes(val string, filters ...string) string  | 字符串按照行过滤，去除匹配的行，支持多个 filter'},
        ...
        {text: 'append_by_lines "APPEND_STR" STR', displayText: 'append_by_lines(appendStr string, message string) string  | 多行字符串每行后面追加字符串'},
        {text: 'json JSONSTR', displayText: 'json(content string) string  |  JSON 字符串格式化'},
        {text: 'json_pretty JSONSTR', displayText: 'json_pretty(content string) string  |  JSON 字符串格式化'},
        ...
        {text: 'json_flatten JSONSTR MAX_LEVEL', displayText: 'json_flatten(body string, maxLevel int) []jsonutils.KvPair  |  将 body 解析为 json，然后转换为扁平化的键值对返回'},
        {text: 'json_flatten_str JSONSTR MAX_LEVEL', displayText: 'json_flatten_str(body string, maxLevel int) string  |  将 body 解析为 json，然后转换为扁平化的字符串返回'},
        {text: 'json_flatten_str_noempty JSONSTR MAX_LEVEL', displayText: 'json_flatten_str_noempty(body string, maxLevel int) string  |  将 body 解析为 json，然后转换为扁平化的字符串返回，同时移除所有空值'},
        {text: 'json_fields_filter JSONSTR KEYS', displayText: 'json_filter(body string, keys ...string) string  |  将 body 解析为 json，然后转换为扁平化的字符串，只保留指定的字段列表，空值将被移除'},
        ...
        {text: 'json_fields_cutoff_str LENGTH JSON_STR', displayText: 'json_fields_cutoff_str(length int, body string) string | 对 JSON 字符串扁平化，然后对每个 KV 截取指定长度，输出字符串'},
